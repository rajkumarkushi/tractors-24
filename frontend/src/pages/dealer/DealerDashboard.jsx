...
const DealerDashboard = () => {
  ...
  const drawer = (
    <Box>
      <Toolbar>
        <Typography variant="h6" noWrap component="div" sx={{ color:'#116978' }}>
          {translations.dealerDashboard}
        </Typography>
      </Toolbar>
      <Divider />
      <List>
        {menuItems.map((item) => (
          ...
          >
            <ListItemIcon sx={{ color: activeTab === item.id ? '#168d91' : 'inherit' }}>
              {item.icon}
            </ListItemIcon>
            <ListItemText 
              primary={item.label}
              sx={{ color: activeTab === item.id ? '#168d91': 'inherit' }}
            />
          </ListItem>
        ))}
      </List>
